-- P0 Critical Fix: Database foreign key relationships and signal queries
-- Fix the duplicate foreign key issue in signals table
ALTER TABLE signals DROP CONSTRAINT IF EXISTS signals_asset_id_fkey;

-- Add proper foreign key with specific name
ALTER TABLE signals 
ADD CONSTRAINT fk_signals_asset_id FOREIGN KEY (asset_id) REFERENCES assets(id);

-- Fix signal_followers table foreign key
ALTER TABLE signal_followers DROP CONSTRAINT IF EXISTS signal_followers_signal_id_fkey;

-- Add proper foreign key with specific name  
ALTER TABLE signal_followers
ADD CONSTRAINT fk_signal_followers_signal_id FOREIGN KEY (signal_id) REFERENCES signals(id);
